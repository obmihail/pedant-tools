$(document).ready(function() {
		...
		$(".delete-browser").click(function() {
			...
			for(var modename in config.modes ) {
				...
				if ( config.modes[modename].length < 1 && '*' != modename )
				{
		...
		$(".save-browser").click(function() {
			...
			if ( unid === undefined )
			{
				...
				pos = config.modes['*'].map(function(browser) { return browser.unid.toLowerCase(); }).indexOf( data.unid.toLowerCase() );
				if ( pos != -1 ) return alert( "Browser with this unid already exists")
				config.modes['*'].push( data )
			}
		...
		$(".start-project").click(function() {
			...
			if ( $("li.custom-mode.active").length > 0 )
			{
				...
				$('.browsers-list input:checked').each( function() { unids.push(  $(this).val() ) });
     			...
     			custom_config.modes['*'] = config.modes['*'].filter( function(browser){  
     				return ( unids.indexOf( browser.unid ) != -1 )
     			})
     			if ( custom_config.modes['*'].length < 1 )
     			{
     				$(document).trigger("add-alerts", [
				...
				var data = { mode: '*', config: custom_config }
		...
		$(".save-mode").click(function() {
			...
			browsers_for_mode = config.modes['*'].filter( function(browser){ return checked_browsers_unids.indexOf( browser.unid ) != -1 } )
			//check fields values
			...
			if ( name.toLowerCase() === '*' ) return alert( "Can not change default mode")
